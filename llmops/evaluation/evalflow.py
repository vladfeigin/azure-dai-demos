...
def verify_prompty_files(evaluators: dict):
    for name, evaluator in evaluators.items():
        try:
            ...
            logger.warning(f"Evaluator '{name}' does not have a PROMPTY_FILE attribute.")
...
def eval_batch(batch_output: pd.DataFrame, dump_output: bool = False):
    ...
    #eval_res = pd.DataFrame(columns=["question", "ground_truth", "context", "response", "groundedness", "relevance", "similarity", "coherence"])
